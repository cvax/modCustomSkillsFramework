...
{	
	...
	event OnSpawned( spawnData : SEntitySpawnData )
	{
		...
		{
			if (StringToInt(theGame.GetInGameConfigWrapper().GetVarValue(customSkills[i].groupName, customSkills[i].groupEnableVar)) == 0)
				AddTimer('CustomSkillsDisable', 1.0, false);
			else if (StringToInt(theGame.GetInGameConfigWrapper().GetVarValue(customSkills[i].groupName, customSkills[i].groupEnableVar)) == 1)
				AddTimer('CustomSkillsCheck', 1.0, false);
			else if (StringToInt(theGame.GetInGameConfigWrapper().GetVarValue(customSkills[i].groupName, customSkills[i].groupEnableVar)) == 2)
				AddTimer('CustomSkillsUninstall', 1.0, false);
		}
	...
	timer function CustomSkillsCheck(dt : float, id : int)
	{
		Debug_FixCustomSkills();
	}

	timer function CustomSkillsUninstall(dt : float, id : int)
	{
		((W3PlayerAbilityManager)abilityManager).UninstallCustomSkills();
	}

	timer function CustomSkillsActionThisRunMsg(dt : float, id : int)
	{
		// 0 = disabled, 1 = installed, 2 = uninstalled, 3 = did nothing, 4 = repaired
		((W3PlayerAbilityManager)abilityManager).CustomSkillsActionThisRunMsg();
	}
	// Console command for Skills Framework
	public function Debug_ListCustomSkills(optional abilityName : string)
	{
		((W3PlayerAbilityManager)abilityManager).ListCustomSkills(abilityName);
	}

	public function Debug_FixCustomSkills(optional force : bool)
	{
